...
| `STOCK_INITIAL` | `8` | Baseline units per SKU used to compute `stock_level` (`ok` / `warning` / `critical`) on each detected bounding box. |

<Note>
`STOCK_INITIAL` is also defined independently in `inventory_engine.py` (see below). Both constants must agree. The detection engine uses it only for visual stock-level colouring on debug frames; the inventory engine uses it to track real stock counts.
</Note>
...
| `STOCK_INITIAL` (detection) | `detection_engine.py` | `8` | No | Baseline units per SKU for stock-level colouring |
